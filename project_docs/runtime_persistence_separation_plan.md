...
- canonical authoring refresh now rebuilds the canonical slice over persisted knowledge concepts, then syncs the merged runtime view back to SQLite
...
- storage mode: in-memory thread-backed job store
...
- `/observability/mapping-jobs/runtime` exposes the live in-memory job pressure and the currently met durable-backend triggers
